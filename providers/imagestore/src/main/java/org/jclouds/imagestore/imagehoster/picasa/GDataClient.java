...
public abstract class GDataClient {
    ...
    protected final <T> T executePost(GoogleUrl url, AbstractHttpContent content, Class<T> parseAsType)
        throws IOException {
        prepareUrl(url, parseAsType);
    ...
    protected final <T> T executePatchRelativeToOriginal(GoogleUrl url, AbstractHttpContent patchContent,
        Class<T> parseAsType, String etag) throws IOException {
        prepareUrl(url, parseAsType);
